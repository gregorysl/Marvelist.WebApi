...
export const seriesDetailsReducer = (state = initialDetailState, action) => {
	switch (action.type) {
		case consts.FETCH_SERIES_BY_ID_SUCCESS: {
			let comics = action.seriesDetails.comics.map((x) => ({
				...x,
				loading: false,
			}));
		...
		case consts.FOLLOW_COMIC: {
			if (action.place == PLACE.SERIES) {
				debugger;
				let idx = state.comics.findIndex((x) => x.id == action.id);
				const newData = update(state, {
					comics: { [idx]: { loading: { $set: true } } },
				});
				return { ...newData };
			}
			return state;
...
export const weekComicsReducer = (
	state = { loading: true, comics: [loadingCardState] },
	action
) => {
	switch (action.type) {
		case consts.WEEK_SUCCESS:
			return {
				...state,
				comics: action.data.map((x) => ({ ...x, loading: false })),
				loading: false,
			};
		case consts.CARD_LOADING:
			return { ...state, loading: true, comics: [loadingCardState] };
		case consts.FOLLOW_COMIC: {
			if (action.place == PLACE.WEEK) {
				debugger;
				let idx = state.comics.findIndex((x) => x.id == action.id);
				const newData = update(state, {
					comics: { [idx]: { loading: { $set: true } } },
